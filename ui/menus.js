  ...
  function createSporelingMenus(options) {
    ...
    function title() {
      ...
      return '<section class="sd-card sd-title-card">' +
        ...
        '<div class="sd-segmented" role="group" aria-label="Run mode">' + button("mode-adventure", "Adventure", timed ? "" : "is-selected") + button("mode-speedrun", "Timed run", timed ? "is-selected" : "") + '</div><p class="sd-mode-copy">' + (adventure ? "Explore 20 chambers and five guardians, with checkpoints and conversations." : "Race the original 15-chamber course against the clock.") + '</p>' +
    ...
    function localPanel(kind) {
      if (kind === "help") {
        var lines = state.touch ? [['Move','Use your ' + (state.leftHanded ? 'right' : 'left') + ' thumb on Move. Push left or right; release to stop.'],['Jump + dash','Hold Jump with your ' + (state.leftHanded ? 'left' : 'right') + ' thumb. While holding, swipe toward your destination and release to dash. One thumb does both.'],['Jump only','Tap for a short hop; hold for height. Keep your thumb still to jump without dashing.'],['Aim','Swipes choose one of 8 directions. Bring your thumb back to where you pressed to cancel the dash.'],['Dash only','Tap Dash to use your movement direction, or swipe and release to aim.'],['Slam','Tap Slam in air (unlocked in Underfield)'],['Burst','Burst lights up when charged (Adventure only)']] : state.gamepad ? [['Move','Left stick or D-pad'],['Jump','A'],['Dash','X or RB'],['Slam','Down + Dash'],['Burst','Y when fully charged (Adventure only)'],['Fullscreen','F']] : [['Move','A / D or Left / Right'],['Jump','Right mouse (hold for height). Space / Z / K also work.'],['Dash','Aim with the pointer and left-click. Keyboard: aim with WASD or arrows, then Shift / J. A / D also move horizontally.'],['Slam','Down + Dash (when unlocked)'],['Burst','Q when fully charged (Adventure only)'],['Music','M'],['Fullscreen','F']];
