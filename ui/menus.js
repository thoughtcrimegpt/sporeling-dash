  ...
  function createSporelingMenus(options) {
    ...
    function title() {
      ...
      return '<section class="sd-card sd-title-card">' +
        '<div class="sd-brand">SPORELING <span>DASH</span></div><p class="sd-purpose">Air-dash to grow a temporary mushroom platform, then climb through the living cave.</p>' +
        '<div class="sd-segmented" role="group" aria-label="Run mode">' + button("mode-adventure", "Adventure", timed ? "" : "is-selected") + button("mode-speedrun", "Timed run", timed ? "is-selected" : "") + '</div><p class="sd-mode-copy">' + (adventure ? "Explore the story at your pace, with checkpoints and conversations." : "Race the full cave against the clock, with comparable leaderboard times.") + '</p>' +
        ...
        '<div class="sd-title-tools">' + button("help", "Controls & help", "sd-button-secondary", "Open controls help") + button("settings", "Settings", "sd-button-secondary", "Open settings") + '</div>' +
        optionsSection() +
        '<p class="sd-hint">' + (v.touch ? "Tap an option. Your adventure starts with Play." : "Move with arrows or stick · Confirm with Enter, A, or tap") + '</p>' +
        '</section>';
    }
    function pause() {
      var v = state;
      return '<section class="sd-card sd-pause-card">' + heading("Paused", v.levelTitle || "Current chamber") +
        ...
        '<button type="button" class="sd-button sd-button-secondary" data-local="help">Controls & help</button>' +
        ...
        '<p class="sd-hint">Choose an action, or press Pause to return to the game.</p></section>';
    }
    function notes() {
    ...
    function localPanel(kind) {
      if (kind === "help") { var lines = state.touch ? [['Move','Drag Move to aim'],['Jump','Tap Jump, release and tap again to flutter (Adventure only)'],['Dash','Aim Move then tap Dash'],['Slam','Tap Slam in air (unlocked in Underfield)'],['Burst','Burst lights up when charged (Adventure only)']] : state.gamepad ? [['Move','Left stick or D-pad'],['Jump','A, flutter in Adventure only'],['Dash','X or RB'],['Slam','Down + Dash'],['Burst','Y when fully charged (Adventure only)']] : [['Move','A / D or Left / Right'],['Jump','Space / Z / K, release then press again to flutter (Adventure only)'],['Dash','Shift or J'],['Slam','Down + Dash'],['Burst','Q when fully charged (Adventure only)'],['Music','M']]; return '<div class="sd-modal" role="dialog" aria-modal="true" aria-labelledby="sd-modal-title"><div class="sd-modal-card">' + heading("Controls", "HOW TO MOVE").replace('<h1>Controls</h1>','<h1 id="sd-modal-title">Controls</h1>') + '<div class="sd-help-grid">' + lines.map(function (p) { return '<p><b>' + text(p[0]) + '</b><span>' + text(p[1]) + '</span></p>'; }).join("") + '</div><p class="sd-hint">' + (state.touch ? "Touch controls stay visible while you play." : "Fluttering and Burst are Adventure-only abilities." ) + '</p>' + button("close", "Got it", "sd-button-primary") + '</div></div>'; }
      if (kind === "settings") return '<div class="sd-modal" role="dialog" aria-modal="true" aria-labelledby="sd-modal-title"><div class="sd-modal-card">' + heading("Settings", "MAKE THE CAVE YOURS").replace('<h1>Settings</h1>','<h1 id="sd-modal-title">Settings</h1>') + '<div class="sd-stack">' + button("reducedmotion", "Reduced motion: " + (state.reducedMotion ? "On" : "Off"), "sd-button-secondary") + button("leftHanded", "Mirrored touch controls: " + (state.leftHanded ? "On" : "Off"), "sd-button-secondary") + button("music", "Music: " + (state.musicOn === false ? "Off" : "On"), "sd-button-secondary") + '</div>' + button("close", "Done", "sd-button-primary") + '</div></div>';
